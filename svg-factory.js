...
import { rand1 } from './puzzle-factory.js'
...
export function getPuzzleSvg(puzzleData){
    ...
    // Randomize whether shape text or color text is on top or bottom
    if(rand1 % 2 == 0){
    ...
    if(rand1 % 2 == 1){
        const topText = createText(puzzleData.text[1].toUpperCase(), puzzleData.colors['shapetext'], textSize, textWeigth, 67)
        const bottomText = createText(puzzleData.text[0].toUpperCase(), puzzleData.colors['colortext'], textSize, textWeigth, 31)
    }

...
export function generateRandomPuzzle(){
    ...
    // randomizes whether shape text or color text appears on the top or bottom
    const rand1 = randomInt(9) + 1
    if(rand1 % 2 == 0){
        const topText = sample(Object.keys(LANG_COLORS))
        const bottomText = sample(SHAPES)
    }
    if(rand1 % 2 == 1){
        const topText = sample(SHAPES)
        const bottomText = sample(Object.keys(LANG_COLORS))
    }
